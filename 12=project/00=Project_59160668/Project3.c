...
int Picture_a (char *first_Name,char *last_Name,int score_Lv1,int score_Lv2,double timeout_Lv1,double timeout_Lv2) {
	...
	FILE*myfile,*datascore;
	char text;
	int result;
	myfile    = fopen("score.dat","a");
	datascore = fopen("score_1.dat","a");
	fprintf(myfile,"\n\t\t%s\t\tScore = %d|Time = %2.2f\t\tScore = %d|Time = %2.2f \n",first_Name,				          score_Lv1,timeout_Lv1,score_Lv2,timeout_Lv2);
	fprintf(datascore,"%d|%d|\n",score_Lv1,score_Lv2);

	int thetext,number1[10000]={0},number2[10000]={0};
	int max=0,min=0,i=0,j=0,k=0;
	FILE*myfile1;
		myfile1=fopen("score_1.dat","r");
		...
		while(result!=EOF) {
			...
			result = fscanf(myfile1,"%d",&thetext);
		}//while(result!=EOF)
         	fprintf(myfile,"number1:%d\nnumber2:%d\n",number1[0],number2[0]);
         	fprintf(myfile,"number1:%d\nnumber2:%d\n",number1[1],number2[1]);
         	fprintf(myfile,"number1:%d\nnumber2:%d\n",number1[2],number2[2]);
         	fprintf(myfile,"number1:%d\nnumber2:%d\n",number1[3],number2[3]);
         	fprintf(myfile,"number1:%d\nnumber2:%d\n",number1[4],number2[4]);
