<?php
  class User{
      ...
      private $Telephone;  //No
            ...
            public function transfer_money($Account,$Accounta,$money,$moneyT){
              session_start();
            	date_default_timezone_set('Asia/Bangkok');
            		$objconnect = mysqli_connect("localhost","root","","mew");
            		$objSQL = $objconnect->query(" select balance from account where account_num = '".$Account."' ");
            		$num = $objSQL->fetch_array();
            		$Balance  = $num['balance']-$money;
            		$moneyT = $moneyT+$money;
            	session_write_close();
            		$datehis = getdate();
            		$dec = " UPDATE account SET balance='$Balance' WHERE account_num='".$Account."' ";
            		$in  = " UPDATE account SET balance='".$moneyT."' WHERE account_num='".$Accounta."' ";
            		$his = " INSERT INTO history (account_num,list,amount,time,date) VALUES ('".$Account."','TRAN','".$money."','".$datehis["hours"].":".$datehis["minutes"].":".$datehis["seconds"]."','".$datehis["year"]."-".$datehis["mon"]."-".$datehis["mday"]."') ";
            	...
            	echo "<script>window.top.replace('user_page.php');</script>";
            	$objconnect->close();
            }
