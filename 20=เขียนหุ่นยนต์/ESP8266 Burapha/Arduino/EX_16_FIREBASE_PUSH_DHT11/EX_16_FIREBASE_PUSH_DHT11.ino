...
void loop() {
  ...
  Firebase.pushFloat("TEM", temperature); //ให้ทำการ PUSH ข้อมูลที่ออปเจตชื่อ TEM ด้วยค่า temperature
  if (Firebase.failed()) { //ถ้า Firebase เชื่อมต่อล้มเหลว
      Serial.print("push /TEM failed:");
      Serial.println(Firebase.error());  
      return;
  }

  Firebase.pushFloat("HUM", humidity); //ให้ทำการ PUSH ข้อมูลที่ออปเจตชื่อ HUM ด้วยค่า humidity
  if (Firebase.failed()) { //ถ้า Firebase เชื่อมต่อล้มเหลว
      Serial.print("push /HUM failed:");
      Serial.println(Firebase.error());  
      return;
  }

  Firebase.pushString("TIME", timestamp); //ให้ทำการ PUSH ข้อมูลที่ออปเจตชื่อ TIME ด้วยค่า timestamp
  if (Firebase.failed()) { //ถ้า Firebase เชื่อมต่อล้มเหลว
      Serial.print("push /TIMEUM failed:");
      Serial.println(Firebase.error());  
      return;
  }
