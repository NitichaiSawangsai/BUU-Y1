...
void loop() {
  ...
  Firebase.setInt("RandomNumber", i); //ให้ทำการ SET ข้อมูลที่ออปเจตชื่อ RandomNumber ด้วยค่า i
  if (Firebase.failed()) { //ถ้า Firebase เชื่อมต่อล้มเหลว
      Serial.print("set /RandomNumber failed:");
      Serial.println(Firebase.error());  
      return;
  }
  Serial.print("set /RandomNumber to ");
  Serial.println(Firebase.getInt("RandomNumber"));      //ให้คำการ GET ข้อมูลจากออปเจคชื่อ RandomNumber แล้วแสดงผลผ่าน Serial Port
